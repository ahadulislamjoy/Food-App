package com.example.foodappex1.Domain;

public class PopularDomain {
    private String title;
    private int pic;
    private String description;
    private Double fee;
    private String numberInCart;

    public PopularDomain(String title, int pic, String description, Double fee) {
        this.title = title;
        this.pic = pic;
        this.description = description;
        this.fee = fee;
    }

    public PopularDomain(String title, int pic, String description, Double fee, String numberInCart) {
        this.title = title;
        this.pic = pic;
        this.description = description;
        this.fee = fee;
        this.numberInCart = numberInCart;
    }

    public String getTitle() {
        return title;
    }

    public void setTitle(String title) {
        this.title = title;
    }

    public int getPic() {
        return pic;
    }

    public void setPic(int pic) {
        this.pic = pic;
    }

    public String getDescription() {
        return description;
    }

    public void setDescription(String description) {
        this.description = description;
    }

    public Double getFee() {
        return fee;
    }

    public void setFee(Double fee) {
        this.fee = fee;
    }

    public String getNumberInCart() {
        return numberInCart;
    }

    public void setNumberInCart(String numberInCart) {
        this.numberInCart = numberInCart;
    }
}
